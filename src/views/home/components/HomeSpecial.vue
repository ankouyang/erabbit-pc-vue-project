<template>
    <HomePanel title="最新专题">
      <template #right><XtxMore /></template>
        ...
        <div class="special-list" v-if="resultData.length">
          <div class="special-item" v-for="i in resultData" :key="i.id">
              <RouterLink to="/">
            <img v-lazyload="i.cover" alt />
            <div class="meta">
              <p class="title">
...
import HomeSpecicalSkeleton from './HomeSpecicalSkeleton'
import { findSpecial } from '@/api/home'
export default {
  name: 'HomeSpecial',
  components: { HomePanel, HomeSpecicalSkeleton },
  setup () {
    const { resultData, target } = useLazyData(findSpecial)
    return {
      resultData, target
    }
  }
